...
const getMenuItems = (t: ReturnType<typeof useI18n>) => [
  {
    label: t['com.affine.workspace.members.team.invite.expiration-date']({
      number: '1',
    }),
...
export const LinkInvite = ({
  ...
  const expireTime = useMemo(() => {
    return t['com.affine.workspace.members.team.invite.expire-at']({
    ...
    <>
      <div className={styles.invitationLinkContainer}>
        <div className={styles.modalSubTitle}>
          {t['com.affine.workspace.members.team.invite.link-expiration']()}
        </div>
        {invitationLink ? (
          <Input
            value={expireTime}
      ...
      <div className={styles.invitationLinkContainer}>
        <div className={styles.modalSubTitle}>
          {t['com.affine.workspace.members.team.invite.invitation-link']()}
        </div>
          ...
          {invitationLink ? (
            <>
              <Button onClick={onCopy} variant="secondary">
                {t['com.affine.workspace.members.team.invite.copy']()}
              </Button>
              <IconButton icon={<CloseIcon />} onClick={onReset} />
            </>
          ) : (
            <Button onClick={onGenerate} variant="secondary">
              {t['com.affine.workspace.members.team.invite.generate']()}
            </Button>
          )}
        </div>
        <p className={styles.invitationLinkDescription}>
          {t[
            'com.affine.workspace.members.team.invite.invitation-link.description'
          ]()}
        </p>
      </div>
    </>
