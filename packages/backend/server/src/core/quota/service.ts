...
export class QuotaService {
  ...
  async getWorkspaceQuota(workspaceId: string): Promise<WorkspaceQuota> {
    ...
    if (!quota) {
      // [SELFHOST PATCH] En self-hosted todos los workspaces tienen plan de equipo
      // sin restricción de miembros — no se requiere licencia externa.
      if (env.selfhosted) {
        ...
        const teamQuota = await this.models.workspaceFeature.getQuota(workspaceId);
        if (teamQuota) return teamQuota.configs;
      }
  ...
  async tryCheckSeat(workspaceId: string, excludeSelf = false) {
    // [SELFHOST PATCH] En modo self-hosted no hay licencia de asientos:
    // cualquier cantidad de miembros puede unirse a cualquier workspace.
